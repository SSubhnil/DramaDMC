...
# os.environ["TRITON_DISABLE_AUTOTUNER"] = "1"
...
from envs.dmc_wrapper import DMControl, DMControlVecEnv
...
def joint_train_world_model_agent(config, logdir,
    ...
    is_vectorized = config.JointTrainAgent.NumEnvs > 1
    ...
    # First, reset the environment to get initial observations for all environments
    if is_vectorized:
        observations, infos = env.reset()
        current_obs = observations  # Already a list/array of observations from vectorized env
    else:
        # For backward compatibility with single env
        observation, info = env.reset()
        current_obs = [observation] * config.JointTrainAgent.NumEnvs

    sum_rewards = [0] * config.JointTrainAgent.NumEnvs  # Track rewards per environment
    # Separate context queues for each environment
    context_obs_list = [deque(maxlen=config.JointTrainAgent.RealityContextLength) for _ in
                        range(config.JointTrainAgent.NumEnvs)]
    context_action_list = [deque(maxlen=config.JointTrainAgent.RealityContextLength) for _ in
                           range(config.JointTrainAgent.NumEnvs)]
    episode_lengths = [0] * config.JointTrainAgent.NumEnvs  # Track episode lengths
    ...
    for total_steps in tqdm(range(config.JointTrainAgent.SampleMaxSteps // config.JointTrainAgent.NumEnvs),
        ...
        for env_idx in range(config.JointTrainAgent.NumEnvs):
            if replay_buffer.ready('world_model'):
                world_model.eval()
                agent.eval()
                with torch.no_grad():
                    context_obs = context_obs_list[env_idx]
                    context_action = context_action_list[env_idx]

                    if len(context_action) == 0:
                        action = env.action_space.sample()
                    else:
                        context_latent = world_model.encode_obs(
                            torch.cat(list(context_obs), dim=1).to(world_model.device))
                        # model_context_action = np.stack(list(context_action))
                        action_arrays = []
                        for act in context_action:
                            if np.isscalar(act):
                                action_arrays.append(np.array([act]))
                            else:
                                action_arrays.append(np.array(act).reshape(1, -1)[0])
                        model_context_action = np.stack(action_arrays)
                ...
                context_obs_list[env_idx].append(
                    rearrange(torch.Tensor(current_obs[env_idx]).to(world_model.device), "H W C -> 1 1 C H W") / 255)
        ...
        observations, rewards, dones, infos = env.step(np.array(actions))
        # print(f"Step {total_steps}: Finished environment step")

        # Process results for each environment
        for env_idx in range(config.JointTrainAgent.NumEnvs):
            # Add to replay buffer
            replay_buffer.append(current_obs[env_idx], actions[env_idx], rewards[env_idx],
                                 infos[env_idx]['is_terminal'])
            if replay_buffer.ready('world_model'):
                context_obs_list[env_idx].append(
                    rearrange(torch.Tensor(current_obs[env_idx]).to(world_model.device),
                              "H W C -> 1 1 C H W") / 255)
                context_action_list[env_idx].append(actions[env_idx])

            # Track rewards and update current observation
            sum_rewards[env_idx] += rewards[env_idx]
            current_obs[env_idx] = observations[env_idx]
            episode_lengths[env_idx] += 1

            if dones[env_idx]:
                logger.log(f"episode/score", sum_rewards[env_idx], global_step=total_steps)
                logger.log(f"episode/length", episode_lengths[env_idx], global_step=total_steps)
                ...
                sum_rewards[env_idx] = 0
                episode_lengths[env_idx] = 0
                context_obs_list[env_idx].clear()
                context_action_list[env_idx].clear()

        train_freq = max(1, config.JointTrainAgent.TrainDynamicsEverySteps // config.JointTrainAgent.NumEnvs)

        if replay_buffer.ready(
                'world_model') and total_steps % train_freq == 0 and total_steps <= config.JointTrainAgent.FreezeWorldModelAfterSteps:
        ...
        if replay_buffer.ready(
                'behaviour') and total_steps % train_freq == 0 and total_steps <= config.JointTrainAgent.FreezeBehaviourAfterSteps:
        ...
        if config.Evaluate.DuringTraining and total_steps > 0 and total_steps % (
                config.Evaluate.EverySteps // config.JointTrainAgent.NumEnvs) == 0 and \
